...
static uint8_t gu8_ModuleStatus = ZERO;
...
uint8_t Timer_Init (Timer_Cfg_ST *Timer_Info)
{
	...
	{
		Error |= E_NOK;
		Error_Push(TIMERS_MODULE,ERROR_NULL_POINTER);
	}

	else
	{
		if (TIMER_MODE == Timer_Info->Timer_Mode)
		{
			switch (Timer_Info->Timer_Ch)
			{
			case Timer_0:
				if (ZERO == (GET_BIT(gu8_ModuleStatus,BIT0)))
				{
				...
				{
					Error_Push(TIMERS_MODULE,ERROR_ALREADY_INITIALIZED);
					Error |= E_NOK;
				}
				if (E_OK == Error)
				{
					SET_BIT(gu8_ModuleStatus,BIT0);
				}
				...
				/*********************************************************************************************************************************/
			case Timer_1:
				if (ZERO == (GET_BIT(gu8_ModuleStatus,BIT1)))
				{
				...
				/************************************************************************************************************************************/
			case Timer_2:
				if (ZERO == (GET_BIT(gu8_ModuleStatus,BIT2)))
				{
		...
		else if (COUNTER_MODE == Timer_Info->Timer_Mode)
		{
			Error_Push(TIMERS_MODULE,ERROR_NOT_IMPLEMENTED);
			Error |= E_NOK;
		}
		else
		{
...
uint8_t Timer_Start (uint8_t Timer_Chan, uint16_t Timer_Count_No)
{
	...
	case Timer_0:
		if (ZERO == (GET_BIT(gu8_ModuleStatus,BIT0)))
		{
			Error_Push(TIMERS_MODULE,ERROR_NOT_INITIALIZED);
			Error |= E_NOK;
		}
		else
		{
...
uint8_t Timer_Stop (uint8_t Timer_Chan)
{
	...
	{
	case Timer_0:
		if (ZERO == (GET_BIT(gu8_ModuleStatus,BIT3)))
		{
			Error_Push(TIMERS_MODULE,ERROR_NO_START);
			Error |= E_NOK;
		}
		else
		{
			CLEAR_BIT(gu8_ModuleStatus,BIT3);
	...
	case Timer_1:
		if (ZERO == (GET_BIT(gu8_ModuleStatus,BIT4)))
		{
			Error_Push(TIMERS_MODULE,ERROR_NO_START);
			Error |= E_NOK;
		}
		...
		{
			CLEAR_BIT(gu8_ModuleStatus,BIT4);
	...
	case Timer_2:
		if (ZERO == (GET_BIT(gu8_ModuleStatus,BIT5)))
		{
			Error_Push(TIMERS_MODULE,ERROR_NO_START);
			Error |= E_NOK;
		}
		...
		{
			CLEAR_BIT(gu8_ModuleStatus,BIT5);
			TCCR2 &= T2_NO_CLOCK;
		}
...
uint8_t Timer_Set_Value (uint8_t Timer_Chan, uint16_t Timer_Value)
{
	...
	case Timer_0:
		...
		{
			if (TIMER0_MAX_NUMBER_OF_COUNTS > Timer_Value)
				TCNT0 = Timer_Value;
			else
			{
				Error_Push(TIMERS_MODULE,ERROR_INVALID_PARAMETER);
	...
	case Timer_2:
		...
		{
			if (TIMER2_MAX_NUMBER_OF_COUNTS > Timer_Value)
				TCNT2 = Timer_Value;
			else
			{
				Error_Push(TIMERS_MODULE,ERROR_INVALID_PARAMETER);
