...
static volatile uint8_t g_u8_SOS_counter = ZERO_VALUE;
...
ERROR_STATUS SOS_Create_Task (SOS_tsak_s *SOS_task){
	...
	{
		for( au8_counter = ZERO_VALUE; au8_counter < SOS_BUFFER_SIZE; au8_counter++)
		{
			if (Buffer[au8_counter] == NULL)
			{
				/*		 set struct in buffer		 */
				Buffer[au8_counter] = SOS_task;
...
void Remove_task(SOS_tsak_s *SOS_task){
	...
	for (au8_counter = ZERO_VALUE; au8_counter<SOS_BUFFER_SIZE; au8_counter++)
	{
		if (Buffer[au8_counter]!=NULL&& SOS_task->Task_ID==Buffer[au8_counter]->Task_ID)
		{
			Buffer[au8_counter] = NULL;
		}
	}
...
ERROR_STATUS SOS_Run (void){
	...
	if (g_u8_ISR_flag == TRUE)
	{
		for (count = ZERO_VALUE; count<SOS_BUFFER_SIZE; count++)
		{
			...
			{
				Buffer[count] ->N_OVFs--;
				/****** check if it the time to run the task *****/
				if ( Buffer[count] ->N_OVFs == ZERO_VALUE)
				{
					Buffer[count]-> Ptr_FunctionCall();
					/****** if the function is oneshot will terminat this task *******/
...
void ISR_flag_func(void){
	g_u8_SOS_counter++;
	if (g_u8_SOS_counter == 50) {
		g_u8_ISR_flag = TRUE;
		g_u8_SOS_counter = ZERO_VALUE;
	}
