...
void LCD_displayCharacter(uint8_t data)
{
	...
	CLEAR_BIT(LCD_CTRL_PORT,RW); /* write data to LCD so RW=0 */
	_delay_us(2); /* delay for processing Tas = 50ns */
// 	SET_BIT(LCD_CTRL_PORT,E); /* Enable LCD E=1 */
// 	_delay_us(1); /* delay for processing Tpw - Tdws = 190ns */
	...
	_delay_us(10); /* delay for processing Tdsw = 100ns */
	CLEAR_BIT(LCD_CTRL_PORT,E); /* disable LCD E=0 */
	_delay_us(5); /* delay for processing Th = 13ns */
	SET_BIT(LCD_CTRL_PORT,E); /* Enable LCD E=1 */
	...
	_delay_us(5); /* delay for processing Tdsw = 100ns */
	CLEAR_BIT(LCD_CTRL_PORT,E); /* disable LCD E=0 */
	_delay_us(3); /* delay for processing Th = 13ns */
}
...
// }
void LCD_WriteStringToXY(uint8_t row , uint8_t col , uint8_t* str)
{
	if ( (row == 0) && (col<16) )
	{
		LCD_sendCommand(127+col);
	}
	else if ( (row == 1) && (col<16) )
	{
		LCD_sendCommand(191+col);
	}
...
{
	LCD_WriteStringToXY(row,col,Str); /* go to to the required LCD position */
	//LCD_displayString(); /* display the string */
}
