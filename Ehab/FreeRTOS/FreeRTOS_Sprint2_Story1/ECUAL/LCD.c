...
void LCD_displayCharacter(uint8_t data)
{
	...
	_delay_ms(3); /* delay for processing Tdsw = 100ns */
	CLEAR_BIT(LCD_CTRL_PORT,E); /* disable LCD E=0 */
	_delay_ms(3); /* delay for processing Th = 13ns */
	SET_BIT(LCD_CTRL_PORT,E); /* Enable LCD E=1 */
	_delay_ms(3); /* delay for processing Tpw - Tdws = 190ns */
...
{
	uint8_t u8_StringIndex = 0;
	while(Str[u8_StringIndex] != '\0')
	{
		LCD_displayCharacter(Str[u8_StringIndex]);
		u8_StringIndex++;
	}
}
...
{
	uint8_t au8_EmptyRow[] = "                ";
	LCD_displayStringRowColumn(0,0,au8_EmptyRow);
...
{
	uint8_t au8_EmptyRow[] = "                ";
	LCD_displayStringRowColumn(1,0,au8_EmptyRow);
}
