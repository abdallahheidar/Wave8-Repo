...
void SOS_Start_Timer_Test()
{
	...
	ERR = SOS_Start_Timer(10,SOS_Init_Test,1000,PERIODIC);
	
	if (ERR == 203)
	{
		printf("SOS_Start_Timer Test Case 1 : Passed \n");
	}
	else
	{
		printf("SOS_Start_Timer Test Case 1 : Failed \n");
	}
	...
	SOS_Config.Timer_Channel = TIMER_CH0;
	SOS_Init();
	ERR = SOS_Start_Timer(10,NULL,1000,PERIODIC);
	if (ERR == 201)
	{
		printf("SOS_Start_Timer Test Case 2 : Passed \n");
	}
	else
	{
		printf("SOS_Start_Timer Test Case 2 : Failed \n");
	}
	/********** TEST CASE 3 ***********/
	/* writing a valid code 
	expected output 0 which maps to no error */
	...
	{
		SOS_Start_Timer(10,SOS_Init_Test,1000,PERIODIC);
	}
	ERR = SOS_Start_Timer(10,SOS_Init_Test,1000,PERIODIC);
	if (ERR == 207)
	{
		printf("SOS_Start_Timer Test Case 4 : Passed \n");
	}
	else
	{
		printf("SOS_Start_Timer Test Case 4 : Failed \n");
	}
	printf("\n");
}

void SOS_Stop_Timer_Test()
{
	ERROR_STATUS ERR = E_OK;
	SOS_DeInit();
	...
	SOS_Init();
	SOS_Start_Timer(10,SOS_Init_Test,1000,PERIODIC);
	ERR = SOS_Stop_Timer(20);
	
	if (ERR == 202)
	{
		printf("SOS_Stop_Timer Test Case 1 : Passed \n");
	}
	else
	{
		printf("SOS_Stop_Timer Test Case 1 : Failed \n");
	}
	...
	ERR = SOS_Stop_Timer(10);
	
	if (ERR == 0)
	{
		printf("SOS_Stop_Timer Test Case 2 : Passed \n");
	}
	else
	{
		printf("SOS_Stop_Timer Test Case 2 : Failed \n");
	}
	printf("\n");
}

void SOS_Dispatch_Test()
{
	ERROR_STATUS ERR = E_OK;
	SOS_DeInit();
	
	/********** TEST CASE 1 ***********/
	/* dispatching before initializing the SOS
	expected output 203 which maps module ID + module not initialized */
	
	ERR = SOS_Dispatch();
	if (ERR == 203)
	{
		printf("SOS_Dispatch Test Case 1 : Passed \n");
	}
	else
	{
		printf("SOS_Dispatch Test Case 1 : Failed \n");
	}
	...
	if (ERR == 0)
	{
		printf("SOS_Dispatch Test Case 2 : Passed \n");
	}

	else
	{
		printf("SOS_Dispatch Test Case 2 : Failed \n");
	}
	...
	if (ERR == 0)
	{
		printf("SOS_Dispatch Test Case 3 : Passed \n");
	}
	else
	{
		printf("SOS_Dispatch Test Case 3 : Failed \n");
	}
	printf("\n");
}
