			...
			case UART_PROTOCOL:
				...
				case SPI_PROTOCOL:
					...
					else
					{
						...
						if(spi_init(&st_spi_inits) == E_OK){
							
							/*save state as initialized state*/
							gu8_BCM_SpiState = INITIALIZED;
						}
						
						/*if there's error don't init it*/
						else
						{
							ERR_Status = BCM_ERROR_BASE + ERROR_NOT_INITIALIZED;
						}
				...
				default:
					...
					/*if another module entered then module not implemented*/
					ERR_Status = BCM_ERROR_BASE + ERROR_BCM_PROTOCOL_NOT_IMPLEMENTED;
				break;
		}
	}
...
ERROR_STATUS BCM_DeInit(void)
{
	...
	if((gu8_BCM_TxBuffer_LockState == BCM_TX_BUFFER_LOCKED) || (gu8_BCM_RxBuffer_LockState == BCM_TX_BUFFER_LOCKED))
