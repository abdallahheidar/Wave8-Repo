...
/************************************************************************/
/* TMU_Init
EnmTMUError_t TMU_Init (const PtrSOS_Configtype ConfigPtr )
ConfigPtr Pointer to a selected configuration structure*/
/************************************************************************/

EnmTMUError_t SOS_Init (const PtrSOS_Configtype ConfigPtr )
{  
	 /* error variable for init fun*/	 
	EnmTMUError_t au8_SOSiniterror =E_OK;
	  	
	/* check pointer_config is null or not */
	if(ConfigPtr==NULL)
	{
		/*if null return null error from error handling*/
		au8_SOSiniterror =TMU_INIT_NULL_POINTER; 
	}
	else
	{  
		...
		if(gu8_SOSinitflag==INITIALIZED)
		{
			...
			au8_SOSiniterror=TMU_MULTIPLE_INIT;
		}
		else
		{   
...
EnmTMUError_t SOS_Start_Timer(uint16_t u16_time,ptrtotask pf_task,uint8_t u8_periodicORoneshot)
{   
	/* error variable for sos_start fun initial value no error*/
	EnmTMUError_t au8_SOSstarterror=E_OK;
	
	/*check if module is initialized or not*/
	if(gu8_SOSinitflag==NOTINITIALIZED)
	{
		au8_SOSstarterror=TMU_MODULE_NOTINITIALIZED;
	}
	...
	else
	{
		...
		if(gu8_SOStasks<MAX_NOTASKS)
		{
			...
			{
				au8_SOSstarterror=TMU_NOT_PERODICORONESHOT;
			}
			else
			{
				...
				if(pf_task==NULL)
				{
					au8_SOSstarterror=TMU_FUN_NULL_POINTER;
				}
				else
				{
		...
		{
			au8_SOSstarterror=TMU_BUFFER_IS_FULL;
		}
	}
...
* @return: EnmTMUError_t au8_SOSstoperror code with error code if one occurred
*/

EnmTMUError_t SOS_Stop_Timer(ptrtotask ptrtofun)
{	
	/* error variable for sos_stop fun initial value no error*/
	EnmTMUError_t au8_SOSstoperror=E_OK;
	uint8_t au8_counter;
	...
	{
		au8_SOSstoperror=TMU_BUFFER_IS_EMPTY;
	}
	else
	{
...
* Input : void
* Output: None
* @return: EnmTMUError_t au8_SOSdiserror code with error code if one occurred
*/


EnmTMUError_t SOS_Dispatch(void)
{
    /* error variable for sos_stop fun initial value no error*/
	EnmTMUError_t au8_SOSdiserror=E_OK;
...
* @return: EnmTMUError_t au8_SOSdiserror code with error code if one occurred
*/
EnmTMUError_t SOS_DeInit(void)
{
	/* error variable for sos_dinit fun initial value no error*/
	EnmTMUError_t au8_SOSEROR=E_OK;
   /* clear all global  variable */
    ...
    /*if SOS WAS NOT_INITIALIZED RETURN TMU_MODULE_NOTINITIALIZED*/
	else
	{
		au8_SOSEROR=TMU_MODULE_NOTINITIALIZED;
	}
