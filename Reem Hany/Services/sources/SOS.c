 ...
 *
 */
ERROR_STATUS SOS_Init(SOS_Cfg* SOS_cfg){
	//Define locals
	...
	if(gu8_InitFlag == FALSE){
		...
		if (SOS_cfg != NULL)
		{
			...
			switch(SOS_cfg->TIM_Ch){
				...
				gstr_TimerCfg.Timer_CH_NO = SOS_TIMER_CH0;
				...
				gstr_TimerCfg.Timer_Cbk_ptr= SOS_CheckFlag;

				//ERROR assignment
				aES_errorStatus = Timer_Init(&gstr_TimerCfg);

				gstr_SOScfg.TIM_Ch = SOS_TIMER_CH0;
				gstr_SOScfg.Resolutions = SOS_RESOLUTION;
				break;

				case SOS_TIMER_CH1:
				//Timer initialization
				gstr_TimerCfg.Timer_CH_NO = SOS_TIMER_CH1;
				gstr_TimerCfg.Timer_Polling_Or_Interrupt = TIMER_INTERRUPT_MODE;
				...
				gstr_SOScfg.TIM_Ch = SOS_TIMER_CH1;
				gstr_SOScfg.Resolutions = SOS_RESOLUTION;
				/*//ERROR assignment
				...
				//Timer initialization
				gstr_TimerCfg.Timer_CH_NO = SOS_TIMER_CH2;
				gstr_TimerCfg.Timer_Polling_Or_Interrupt = TIMER_INTERRUPT_MODE;
				...
				gstr_SOScfg.TIM_Ch = SOS_TIMER_CH2;
				gstr_SOScfg.Resolutions = SOS_RESOLUTION;
					...
					switch(gstr_SOScfg.TIM_Ch){
						...
						if(au8_CreatedFlag == FALSE){
							...
							if(gu8_StartedFlag == FALSE){
								//Start the timer
								aES_errorStatus = Timer_Start(SOS_TIMER_CH0,(_8BIT_RESOLUTION_-gstr_SOScfg.Resolutions));
						...
						if(au8_CreatedFlag == FALSE){
							...
							if(gu8_StartedFlag == FALSE){
								//Start the timer
								aES_errorStatus = Timer_Start(SOS_TIMER_CH1,(_16BIT_RESOLUTION_-gstr_SOScfg.Resolutions));
								//Mark that the timer is started successfully
								gu8_StartedFlag = TRUE;
							}
							//In case the timer is started
						...
						if(au8_CreatedFlag == FALSE){
							...
							if(gu8_StartedFlag == FALSE){
								//Start the timer
								aES_errorStatus = Timer_Start(SOS_TIMER_CH2,gstr_SOScfg.Resolutions);
								//Mark that the timer is started successfully
								gu8_StartedFlag = TRUE;
							}
							//In case the timer is started
