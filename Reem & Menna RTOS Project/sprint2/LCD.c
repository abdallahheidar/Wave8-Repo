...
void LCD_setLine(uint8_t u8_line_number){
	switch(u8_line_number)
	{
		case 0:
			LCD_writeCMD(0x80);
			_delay_ms(5);
...
ERROR_STATUS LCD_writeString(char * string)
{
	ERROR_STATUS ERR = ERROR_LCD_MODULE + E_OK;
	uint8_t u8_str_index = 0;
	while(string[u8_str_index] != '\0'){
		LCD_writeChar(string[u8_str_index]);
		u8_str_index++;
	}
	return ERR;
}
