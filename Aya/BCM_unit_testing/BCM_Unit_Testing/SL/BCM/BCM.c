...
gstr_BCM_Config_t gstr_ConfigDeInitPtr ;
		...
		if (ConfigPtr!=NULL)
		{
			...
			switch (ConfigPtr->UART_Channel )
			{
				...
				case UART_Channel_1 :
				{
					...
					gstr_ConfigDeInitPtr.UART_Channel = ConfigPtr->UART_Channel;
					Ret_Error |= E_OK;
					break;
				}
				case UART_Channel_2 :
			...
			switch (ConfigPtr->SPI_Channel )
			{
				...
				case SPI_Channel_1 :
				{
					...
					else
					{
                        ...
                        gstr_ConfigDeInitPtr.SPI_Channel = ConfigPtr->SPI_Channel;
			...
			switch (ConfigPtr->I2C_Channel )
			{
				...
				case I2C_Channel_1 :
				{
					...
					//gstr_ConfigDeInitPtr.I2C_Channel = ConfigPtr.I2C_Channel ;
					Ret_Error |= E_OK;
					break;
				}
				case I2C_Channel_2 :
...
ERROR_STATUS BCM_DeInit (void )
{
	...
	if (BCM_Init_Flag == INITIALIZED)
	{
			switch (gstr_ConfigDeInitPtr.UART_Channel )
			{
				case  UART_NO_Channel :
				{
					Ret_Error |= E_OK;
				...
				case UART_Channel_1 :
				{
					...
					UART1_CFG.USART_MODE = USART_ASYNC_MODE;
					UART1_CFG.USART_PARITY = USART_PARITY_DIS;
					UART1_CFG.USART_RX_TX = USART_RX_TX_Disable;
					UART1_CFG.USART_INTERRUPTS = USART_Interrupt_Disabl;
					Ret_Error |= USART_Init2(& UART1_CFG);
					/* set UART Channel 1 flag */
					gu8_UART_Channel_1 = NOT_INITIALIZED;
					Ret_Error |= E_OK;
					break;
				}
				case UART_Channel_2 :
			...
			switch (gstr_ConfigDeInitPtr.SPI_Channel )
			{
				case SPI_NO_Channel :
				{
					Ret_Error |= E_OK;
				...
				case SPI_Channel_1 :
				{
					/*    SPI INITIALIZATION    */
						...
						Ret_Error |= SPI_Init_(Interrupt_Disable,SPI_Disable,MSB,Master,Leading_Edge_Rising,Leading_Edge_Sample,Frequency_16);

					/* set SPI Channel 1 flag */
					gu8_SPI_Channel_1 = NOT_INITIALIZED;
					Ret_Error |= E_OK;
					break;
					}
				case SPI_Channel_2 :
			...
			switch (gstr_ConfigDeInitPtr.I2C_Channel )
			{
				case I2C_NO_Channel :
				{
					Ret_Error |= E_OK;
				...
				case I2C_Channel_1 :
				{
					//SPI INITIALIZATION
					//Not Handled, for future use
					gu8_I2C_Channel_1 = NOT_INITIALIZED;
					Ret_Error |= E_OK;
					break;
				}
				case I2C_Channel_2 :
