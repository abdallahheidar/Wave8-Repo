...
/* this struct is used to update the status of SOS */
...
/* create SOS buffer to serve functions "array of structures"*/
static str_SOS_Buffer_t arrstr_SOS_Buffer [SOS_BUFFER_SIZE];

/* create global instance of str_FuncStatus_t  */
static str_FuncStatus_t  str_FuncStatus;

/* to count elements in the SOS buffer*/
...
/* this function is used to check if SOS buffer is empty or not */
static ERROR_STATUS Is_Buffer_Empty(void)
	{
...
static ERROR_STATUS Is_Buffer_Full(void)
	{
		uint8_t u8_Ret_Status = FALSE;
		if (gu8_Buffer_Counter == SOS_BUFFER_SIZE)
		{
			u8_Ret_Status=TRUE;
		}
		else
...
ERROR_STATUS Sos_Init (const str_SOS_InitConfig_t * pstr_ConfigPtr )
{
	uint8_t Error = E_OK;
...
ERROR_STATUS Sos_Dispatch(void)
{
	...
	{
	/*Search in SOS buffer if there is function to server this tick */
	for (u8_Buffer_Index=0; u8_Buffer_Index< SOS_BUFFER_SIZE && u8_Flag<gu8_Buffer_Counter; u8_Buffer_Index++)
	{
		if(arrstr_SOS_Buffer[u8_Buffer_Index].u8_Status == USED)
		{
			arrstr_SOS_Buffer[u8_Buffer_Index].u8_Pre_Flag++;

			/*Check if this task will serve this tick*/
			if  (arrstr_SOS_Buffer[u8_Buffer_Index].u8_Pre_Flag == (arrstr_SOS_Buffer[u8_Buffer_Index].u8_Periodicity_Time) )
			{
					
				arrstr_SOS_Buffer[u8_Buffer_Index].Ptr_Consumer();
				arrstr_SOS_Buffer[u8_Buffer_Index].u8_Pre_Flag = ZERO;					
			}
		}
	}
...
ERROR_STATUS SOS_Stop (uint8_t Task_Id)
{
	   ...
	   if (FALSE == Is_Buffer_Empty())
		   {
			   ...
			   for (au8_Search_Loop_Counter = ZERO; au8_Search_Loop_Counter < SOS_BUFFER_SIZE; au8_Search_Loop_Counter++)
			   {
				   /*Check if this task is the desired to stop*/
				   if ((Task_Id == arrstr_SOS_Buffer[au8_Search_Loop_Counter].u8_Task_ID))
				   {
					   /*stop the task*/
					   arrstr_SOS_Buffer[au8_Search_Loop_Counter].u8_Status = NOT_USED;
					   arrstr_SOS_Buffer[au8_Search_Loop_Counter].Ptr_Consumer = NULL;
					   /*decrease the buffer*/
					   gu8_Buffer_Counter --;
					   break;
				   }
...
ERROR_STATUS Sos_Start (gstr_Task_Info_t str_Task_1 )
{
	   ...
	   if (FALSE == Is_Buffer_Full())
		{
		   if (NULL != str_Task_1.Ptr_Func)
		   {
			   ...
			   /* Search for empty location and if the consumer is already started in the SOS buffer*/
			   for (au8_Search_Loop_Counter = ZERO; au8_Search_Loop_Counter < SOS_BUFFER_SIZE; au8_Search_Loop_Counter ++)
			   {
				   /* save the first empty location in SOS buffer*/
				   if ((NOT_USED == arrstr_SOS_Buffer[au8_Search_Loop_Counter].u8_Status) && (aS8_Empty_Buffer_Location == EMPTY_BUFFER_LOCATION_INITIAL_VALUE))
				   {
					   aS8_Empty_Buffer_Location = au8_Search_Loop_Counter;
				   }
				   /*Check if the consumer is pre started with the same ID*/
				   if ((str_Task_1.Task_Id == arrstr_SOS_Buffer[au8_Search_Loop_Counter].u8_Task_ID))
				   {
			   ...
			   if (au8_Already_Started == NOT_INITIALIZED)
			   {
				   ...
				   arrstr_SOS_Buffer[aS8_Empty_Buffer_Location] = str_Buffer;
