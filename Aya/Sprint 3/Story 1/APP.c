...
/* Tasks Periodicities */
#define TASKA_LCD_DISPLAY_PERIODICITY			150
#define TASKB_KEYPAD_GET_KEY_PERIODICITY	   50
#define TASKD_GET_BTN1_STATE_PERIDICITY		50
#define TASKC_UART_DATA_PERIODICITY          50       /* in case this task take action based action based on interrupt event and we don't know,
...
volatile static uint8_t* gptr_UART_Rec_Data;
...
static void TaskA_LCD_Display_Code( void * pvParameters )
{
	...
	for( ;; )
	{
		...
		if (Re_Init_Flag == SET_FLAG && uxBits_BTN != BTN1_EVENT_BIT)
		{
			...
			LCD_Display_String("B: R");
			LCD_Display_String("   K.P: ");
			LCD_GoTo_Row_Col(ROW_1,COL_0);
			LCD_Display_String("UART DATA: ");
		}
	   ...
	   /* In case there are new data sent through UART*/
		if (uxBits_UART == UART_EVENT_BIT && uxBits_BTN != BTN1_EVENT_BIT)
		{
			LCD_GoTo_Row_Col(ROW_1,COL_0);
			LCD_Display_String("UART DATA: ");
			LCD_Display_Char(gptr_UART_Rec_Data);
...
static void TaskC_Get_UART_Data_Code( void * pvParameters )
{
	...
	for (;;)
	{
		...
		{
				gptr_UART_Rec_Data = USART_Receive_NO_POLLING();
