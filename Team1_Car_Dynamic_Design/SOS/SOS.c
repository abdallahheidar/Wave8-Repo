...
/*Configuration structure instance golds the SOS configuration data*/
...
EnmSOSError_t SOS_Init(void)
{
	...
	{
		/*Setting selected timer channel to 64us tick count*/
		switch(sos_cfg.timer_channel)
		{
			case SOS_TIM_CH0:
			Timer_Init(TIMER_CH0 , TIMER_MODE , TIMER_PRESCALER_1024 , TIMER_INTERRUPT_MODE);
			gu8_timer_selectedChannel = TIMER_CH0;
			gu8_timer_tickTime = (sos_cfg.timer_tick_time_ms);
			break;
				
			case SOS_TIM_CH1:
			Timer_Init(TIMER_CH1 , TIMER_MODE , TIMER_PRESCALER_1024 , TIMER_INTERRUPT_MODE);
			gu8_timer_selectedChannel = TIMER_CH1;
			gu8_timer_tickTime = (sos_cfg.timer_tick_time_ms);
			break;

			case SOS_TIM_CH2:
			Timer_Init(TIMER_CH2 , TIMER_MODE , TIMER_PRESCALER_1024 , TIMER_INTERRUPT_MODE);
...
EnmSOSError_t SOS_run(void)
{
	 ...
	 */
	uint16_t au16_systemTick = (uint16_t)( ( (uint32_t)gu8_timer_tickTime * MS_TO_US_CONVERSION ) / TICK_64US );
...
/*
 * Description: Small OS dispatcher function that's used in selecting which task will
				take the cpu
   @param(IN) : N/A
   @param(OUT): N/A
   @param(I/O): N/A
