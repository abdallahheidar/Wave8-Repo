...
ISR(USART_TXC_vect)
{
	if(gaptr_uartInterrupt_CBF[TX_CBF] != 0)
	{
...
ISR(USART_RXC_vect)
{
	if(gaptr_uartInterrupt_CBF[RX_CBF] != 0)
	{
...
ISR(USART_UDRE_vect)
{
	if(gaptr_uartInterrupt_CBF[UDRE_CBF] != 0)
	{
...
ERROR_STATUS UART_Init(UART_cfg_s *pUART_cfg_s)
{
	...
	uint8_t u8_UCSRC_Buffer = UART_RSEL;
 ...
 *
 **/
ERROR_STATUS UART_SendByte(uint8_t u8_Data)
{
 ...
 *
 **/
ERROR_STATUS UART_ReceiveByte(uint8_t *pu8_ReceivedData)
{
