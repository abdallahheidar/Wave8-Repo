...
#define LCD_MAX_R0W			2/*LCD END OF ROWS*/

static uint8_t gu8_lcdBuffer[LCD_ROWS][LCD_COLUMS];
static uint8_t gu8_lcdState;
uint8_t gu8_UARTmoduleInitFlage = FALSE; /*I CANT INIT THIS IN THE INIT FUNCTION AS IT HAS TO CHECK ITS VALUE FOR VERFICATION*/
...
{
	ERROR_STATUS s16_funStatus = OK;
	if (gu8_UARTmoduleInitFlage == FALSE)
	{
...
{
	if (x < LCD_MAX_R0W && y < LCD_MAX_CLUM)
		gu8_lcdBuffer[x][y] = value;
	else
		return LCD_MODULE_ERR + INVALAD_PARAMETER;
}