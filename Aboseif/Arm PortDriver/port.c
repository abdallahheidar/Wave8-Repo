...
void Port_Init(void)
{
	uint8_t i, u8_PortNum, u8_u8_ChannelOffset; 

	for(i=0; i< NUM_OF_ACTIVATED_CHANNELS; i++)
	{
		/* 0 - get the port and the channle */
		
		u8_PortNum= PortDriver_CfgArr[i].Channel / NUM_OF_CHANNELS_IN_PORT;
		u8_ChannelOffset = PortDriver_CfgArr[i].Channel% NUM_OF_CHANNELS_IN_PORT;
