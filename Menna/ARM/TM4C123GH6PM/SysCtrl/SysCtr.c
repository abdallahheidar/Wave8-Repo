...
#define RCGC_REG(OFFSET) *((volatile uint32_t *)(FIRST_RCGC_ADD_REG+OFFSET))


void SysCtr_init(void)
{
	uint8_t counter ,offset = 0 ,RCGC_BIT_NO = 0 ;
	...
	for(counter=0; counter <NUM_OF_PRIPHERALS ;counter++)
	{
		...
		if(counter == 2||counter ==8||
			counter ==14||counter ==16||counter ==24||
			counter ==28||
		...
		{
			SET_BIT(RCGC_REG(offset),RCGC_BIT_NO);
